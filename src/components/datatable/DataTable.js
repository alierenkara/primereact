...
export class DataTable extends Component {
  ...
  getMultiSortMeta() {
    return this.props.onSort
      ? this.props.multiSortMeta
      : this.state.multiSortMeta;
  }
    ...
    if (this.columnWidthsState) {
      ...
      if (this.props.scrollable) {
        let headerCols = DomHandler.find(
          this.container,
          ".p-datatable-scrollable-header-table > colgroup > col"
        );
        let bodyCols = DomHandler.find(
          this.container,
          ".p-datatable-scrollable-body-table > colgroup > col"
        );
        ...
        headers.map(
          (header, index) => (header.style.width = widths[index] + "px")
        );
      }
    }
  }
    ...
    if (this.props.sortMode === "multiple") {
      ...
      if (multiSortMeta && multiSortMeta instanceof Array) {
        const sortMeta = multiSortMeta.find(
          (sortMeta) => sortMeta.field === sortField
        );
        sortOrder = sortMeta
          ? this.getCalculatedSortOrder(sortMeta.order)
          : sortOrder;
      }
      ...
      sortOrder =
        this.getSortField() === sortField
          ? this.getCalculatedSortOrder(this.getSortOrder())
          : sortOrder;
  ...
  getCalculatedSortOrder(currentOrder) {
    return this.props.removableSort
      ? currentOrder
        ? currentOrder < 0
          ? 0
          : -1
  ...
  sortMultiple(data, multiSortMeta) {
    ...
    if (this.columnSortable && this.columnSortFunction) {
      const meta = multiSortMeta.find(
        (meta) => meta.field === this.columnField
      );
  ...
  onColumnResizeStart(event) {
    ...
    this.lastResizerHelperX =
      event.originalEvent.pageX - containerLeft + this.container.scrollLeft;
  ...
  onColumnResize(event) {
    ...
    this.resizerHelper.style.left =
      event.pageX - containerLeft + this.container.scrollLeft + "px";
  ...
  setScrollableItemsWidthOnExpandResize(column, newColumnWidth, delta) {
    let scrollableView = column
      ? this.findParentScrollableView(column)
      : this.container;
    ...
    const isContainerInViewport =
      this.container.offsetWidth >= scrollableBodyTableWidth;
  ...
  findParentScrollableView(column) {
    if (column) {
      ...
      while (
        parent &&
        !DomHandler.hasClass(parent, "p-datatable-scrollable-view")
      ) {
  ...
  resizeColGroup(table, resizeColumnIndex, newColumnWidth, nextColumnWidth) {
    if (table) {
      ...
      if (colGroup) {
        ...
        throw new Error(
          "Scrollable tables require a colgroup to support resizable columns"
        );
      }
    }
  }
  ...
  onColumnDragStart(event) {
    ...
    this.iconWidth = DomHandler.getHiddenElementOuterWidth(
      this.reorderIndicatorUp
    );
    this.iconHeight = DomHandler.getHiddenElementOuterHeight(
      this.reorderIndicatorUp
    );
  ...
  onHeaderCheckboxClick(event) {
    ...
    if (!event.checked) {
      let visibleData = this.hasFilter()
        ? this.processData()
        : this.props.value;
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      for (let j = 0; j < columns.length; j++) {
        ...
        if (filterMeta) {
          ...
          let filterMatchMode =
            filterMeta.matchMode || col.props.filterMatchMode;
          let filterConstraint =
            filterMatchMode === "custom"
              ? col.props.filterFunction
              : FilterUtils[filterMatchMode];
        ...
        if (
          !col.props.excludeGlobalFilter &&
          this.props.globalFilter &&
          !globalMatch
        ) {
          globalMatch = FilterUtils["contains"](
            ObjectUtils.resolveFieldData(value[i], columnField),
            this.props.globalFilter,
            this.props.filterLocale
          );
        }
  ...
  processData(localState) {
    ...
    if (!this.props.lazy) {
      if (data && data.length) {
        let sortField =
          (localState && localState.sortField) || this.getSortField();
        let sortOrder =
          (localState && localState.sortOrder) || this.getSortOrder();
        let multiSortMeta =
          (localState && localState.multiSortMeta) || this.getMultiSortMeta();
        ...
        let localFilters =
          (localState && localState.filters) || this.getFilters();
  ...
  createScrollableView(
    ...
    return (
      <ScrollableView
        ...
        frozenBody={
          this.props.frozenValue
            ? this.createTableBody(this.props.frozenValue, columns, true)
            : null
        }
    ...
    let className = classNames(
      "p-datatable p-component",
      {
        ...
        "p-datatable-resizable-fit":
          this.props.resizableColumns && this.props.columnResizeMode === "fit",
        ...
        "p-datatable-hoverable-rows":
          this.props.rowHover || this.props.selectionMode,
      },
      this.props.className
    );
    let paginatorTop =
      this.props.paginator &&
      this.props.paginatorPosition !== "bottom" &&
      this.createPaginator("top", totalRecords);
    let paginatorBottom =
      this.props.paginator &&
      this.props.paginatorPosition !== "top" &&
      this.createPaginator("bottom", totalRecords);
    let headerFacet = this.props.header && (
      <div className="p-datatable-header">{this.props.header}</div>
    );
    ...
    if (Array.isArray(columns)) {
      if (this.props.scrollable) {
        this.frozenSelectionMode =
          this.frozenSelectionMode ||
          this.getFrozenSelectionModeInColumn(columns);
        let frozenColumns = this.getFrozenColumns(columns);
        let scrollableColumns = frozenColumns
          ? this.getScrollableColumns(columns)
          : columns;
        ...
        let tableFooter = this.createTableFooter(
          columns,
          this.props.footerColumnGroup
        );
