...
export class ScrollableView extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (!this.props.frozen && this.props.virtualScroll) {
      this.virtualScroller.style.height =
        this.props.totalRecords * this.props.virtualRowHeight + "px";
    }
  ...
  setScrollHeight() {
    if (this.props.scrollHeight) {
      ...
      if (frozenView) {
        let frozenScrollBody = DomHandler.findSingle(
          frozenView,
        ...
        if (this.props.scrollHeight.indexOf("%") !== -1) {
          ...
          let relativeHeight =
            (DomHandler.getOuterHeight(datatableContainer.parentElement) *
              parseInt(this.props.scrollHeight, 10)) /
            100;
  ...
  onBodyScroll() {
    ...
    if (frozenView) {
      frozenScrollBody = DomHandler.findSingle(
        frozenView,
        ".p-datatable-scrollable-body"
      );
    }

    this.scrollHeaderBox.style.marginLeft =
      -1 * this.scrollBody.scrollLeft + "px";
    if (this.scrollFooterBox) {
      this.scrollFooterBox.style.marginLeft =
        -1 * this.scrollBody.scrollLeft + "px";
    }
    ...
    if (this.props.virtualScroll) {
      ...
      if (
        this.scrollBody.scrollTop + viewport >
        ...
        let page =
          Math.floor(
            (this.scrollBody.scrollTop * pageCount) /
